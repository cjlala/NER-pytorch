...
class BiLSTM_CRF(nn.Module):
    ...
    def _viterbi_decode(self, feats):
        ...
        best_path.reverse()
        return path_score, torch.tensor(best_path, dtype=torch.long)
...
if __name__ == "__main__":
    ...
    model = BiLSTM_CRF(len(char2idx), tag2idx, 64, 64)
    ...
    for epoch in range(20):
        ...
        for sent in tqdm(data[:-1]):
            ...
            loss = model.neg_log_likelihood(sentence_in, tags)
            loss.backward()
            optimizer.step()

            _, predicions = model(sentence_in)
